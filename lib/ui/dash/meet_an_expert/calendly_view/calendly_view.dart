...
class CalendlyView extends StatefulWidget {
  const CalendlyView(
      {Key? key, required this.trainer, required this.communityId, required this.refresh, required this.back})
      : super(key: key);
  final Member trainer;
...
class _CalendlyViewState extends State<CalendlyView> {
  ...
  Future listenForUrl(DashProvider dash) async {
      ...
      child: Scaffold(
          ...
          title: Row(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  image: DecorationImage(
                      image: NetworkImage(widget.trainer.profileImage!),
                      fit: BoxFit.cover),
                ),
              ),
              const SizedBox(
              ...
              Column(
                ...
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text("${widget.trainer.firstName} ${widget.trainer.lastName}",
        ...
        body: _error != null
            ...
            : WebView(
                ...
                initialUrl: Uri.encodeFull("${widget.trainer.calendlylink!}?utm_content=${jsonEncode({
                "communityObjectId": widget.communityId,
                "learnerObjectId": user?.learner.id
                })}"),
                onPageFinished: (val)async{
                  await listenForUrl(dash);
                },
